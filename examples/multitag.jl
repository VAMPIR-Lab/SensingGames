...
function make_multitag_sensing_step(agent, num_p, num_e; fov, scale=20.0, offset=2)
    ...
    function rollout_observation(state_dist::StateDist, game_params)
        ...
        obs = mapreduce(hcat, enumerate(id_other_pos)) do (i, id) 
            ...
            μ = gaussian_params[i][1]
            σ2 = gaussian_params[i][2]
            log.(1 ./ quantile .- 1) .* sqrt.(σ2) ./ -1.702 .+ μ
        end
...
function make_multitag_costs(num_p, num_e)
    ...
    for i = 1:num_p
        ...
        function cost_p(hist)
            sum(hist) do distr
                sum((
                    (sum((distr[p_pos] .- distr[e_pos]).^2, dims=2)) .+
    ...
    open("multitag.txt", "a") do file
        for solvers in [
            ...
            for s = 1:20
                ...
                write(file, "Active: $(sum(scores_active)/n_steps/2), Inactive: $(sum(scores_inactive)/n_steps/2)\n")
            end
        end
    end
end
